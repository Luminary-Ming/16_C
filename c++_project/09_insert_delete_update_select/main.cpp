...
int main(void)
{
	...
	// 增删改查
	int index = v3->insert(2233);
	std::cout << "select " << index << " = " << v3->select(index) << std::endl;  // select 5 = 2233
	v3->update(index, 3322);
	std::cout << "select " << index << " = " << v3->select(index) << std::endl;  // select 5 = 3322
	v3->del(0);
	v3->show();
...
*/
	...
	/* ---------- 下标运算符重载测试示例 ---------- */
	VLA v4(3);  // 实例化 v4 对象, 拥有 3 个元素 (栈区对象, 作用域结束时自动销毁)
	v4[0] = 22222223;
	v4[1] = 2222223;
	v4[2] = 222223;
	v4.show();

	const VLA v5(v4);  // 实例化常对象 v5（常对象只能访问常函数）
	std::cout << v5[1] << std::endl;  // 打印：2222223
