...
int queue_en(queue_t *q, const void *data)
{
	...
	memcpy((char *)q->data + q->rear * q->size, data, q->size);
	// q->data 强转为 char * 就能 1 字节偏移
	// q->rear * q->size 队尾标记 * 一个空间的大小 = 要访问的元素在内存中的字节偏移量
	// (char *)q->data + q->rear * q->size  将基地址加上偏移量，得到目标元素的内存地址
...
int queue_de(queue_t *q, void *save)
{
	...
	memcpy(save, (char *)q->data + q->front * q->size, q->size);
	memset((char *)q->data + q->front * q->size, '\0', q->size);  // 清空脏数据 (清理出队的数据)
