...
int mycat(const char *pathname)
{
	...
	while (1)  // 循环读文件内容
	{
		...
		ssize_t read_size = pread(fd, buf, BUF_SIZE, offset);  // 获取实际读到的字节数		  
		...
		pwrite(STDOUT_FILENO, buf, read_size, offset);
