...
{
    struct llist_node *cur = l;  // cur 指向旧链表的每一个节点

    struct llist_node *min_head = malloc(sizeof(struct llist_node));  // min_head 指向小链表, 创建小链表的头
    struct llist_node *min_cur = min_head;  // min_cur 指向小链表当前操作的节点

    struct llist_node *max_head = malloc(sizeof(struct llist_node));  // max_head 指向大链表, 创建大链表的头
    struct llist_node *max_cur = max_head;  // max_cur 指向大链表当前操作的节点
